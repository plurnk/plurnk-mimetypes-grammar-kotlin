#!/usr/bin/env node
// CI byte-identical reproducibility check for kotlin.wasm.
...
const committed = await readFile(path.join(repoRoot, "kotlin.wasm"));
const committedHash = createHash("sha256").update(committed).digest("hex");
console.log(`committed kotlin.wasm sha256: ${committedHash}`);

const work = await mkdtemp(path.join(tmpdir(), "grammar-kotlin-verify-"));
...
console.log(`rebuilt kotlin.wasm sha256: ${rebuiltHash}`);
